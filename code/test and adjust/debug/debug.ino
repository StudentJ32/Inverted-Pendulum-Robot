...
//********************gain design ************************************
float K1_1 = -0.1266;//-0.0175;
float K1_2 = -0.0700;//-0.0175;
float K1_3 = -0.5415;;//-0.4946;
float K1_4 = -0.0207; //-0.0087;
float K2_1 = 0.0487;
float K2_2 = 0.0055;
// ********motor control*******
//*********motor input***********;
...
float J = 0.02;//0.0188; // moment of inertia about rotation axis
...
void loop() {
    ...
    X_act = count_1*2*pi/6400*wheel_r;
    X_act_dot = speed_act_1*wheel_r;
    yaw_act = (count_1-count_2)*2*pi*wheel_r/(6400*D);
    yaw_act_dot = (speed_act_1-speed_act_2)*wheel_r/D;
    torque_sum = -(K1_1*(X_act-X_in)+K1_2*(X_act_dot-X_dot_in)+K1_3*((kalAngleY+Y_offset)*deg2rad)+K1_4*gyroYrate*deg2rad);
    ...
    vol_in_L = MotorVoltageUpdate_L(error_w_dot_L); //vol_in_L+K_a*error_w_dot_L+K_b*old_error_w_dot_L;
    vol_in_R = MotorVoltageUpdate_R(error_w_dot_R);//vol_in_R+K_a*error_w_dot_R+K_b*old_error_w_dot_R;
      if(vol_in_L<0) {
      M_L->run(FORWARD);
      }
      if(vol_in_L>0) {
  ...
  PWM_val_2 = volt2pwm_2(abs(vol_in_R));
...
void getMotorData()  {
  ...
  w_dot_m_L = (float) 0.02*(3*speed_act_1-4*speed_act_1_old+speed_act_1_oold)/(dt+dt_old);
  w_dot_m_R = (float) 0.02*(3*speed_act_2-4*speed_act_2_old+speed_act_2_oold)/(dt+dt_old);
...
double MotorVoltageUpdate_L (float e0)  {
float u;
static double e1_L=0;
float kp =3;
float ki = 200;

u = kp*e0+ki*(e1_L+e0)*LOOPTIME/2000; 
e1_L=e0;
return u;
}

double MotorVoltageUpdate_R (float e0)  {
float u;
static double e1_R=0;
float kp =3;
float ki = 200;

u = kp*e0+ki*(e1_R+e0)*LOOPTIME/2000; 
e1_R=e0;
return u;
}
